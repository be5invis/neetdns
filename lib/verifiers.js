...
class HTTPSVerifier extends Verifier {
	constructor(ip, hostname) {
		super(ip);
		this.hostname = hostname;
	}
...
exports.fromString = function(manner, ip, domain) {
	if (typeof manner === "string") {
		...
		} else if (manner === "https") {
			return new HTTPSVerifier(ip, domain);
		} else if (manner === "yes") {
	...
	} else if (manner.by === "https") {
		return new HTTPSVerifier(ip, manner.hostname || domain);
	} else {
		return new Verifier(ip);
	}
};
